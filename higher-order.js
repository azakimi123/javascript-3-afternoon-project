...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(strong) {
  return strong.CP > 200;
})
...
let orderTotals = orders.map(function(price) {
  let newArr = [];
  newArr += (price.price * (1 + price.tax));
  newArr = parseFloat(newArr).toFixed(2);
  return parseFloat(newArr);
});

// let orderTotals = newArr.reduce(total, price) => {
//   return total+=price;
// }
...
// function bobsTotal (purchases) {
//   let newArr = [];
//   for (let i = 0; i < purchases.length; i++) {
//     if (purchases[i].owner === 'Bob') {
//       newArr.push(purchases[i].price);
//       let total = newArr.reduce(function(acc, curr) {
//         return acc + curr;
//       })
//     }
//   }
...
// let bobsTotal = priceArr.reduce(total, price) {
//   let sum = 0;
//   sum = total += price;
//   return sum;
// }
